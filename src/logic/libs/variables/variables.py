...
def iniciar(modulos: List[Any]):
    ...
    for clase in modulos:
        ...
        config.VARIABLES_PREDEFINIDAS.update(nuevas_variables)

        config.LISTA_ENUMS.extend(clase.Variable)
        config.NO_MOSTRAR.extend(clase.no_mostrar)
...
def dame(variable: Enum) -> str:
    ...
    """
    valor_de_diccionario = config.VARIABLES_PREDEFINIDAS.get(variable.value)
...
def variables_cargadas() -> Dict[str, str]:
    ...
    return {
        ...
        for clave in config.LISTA_ENUMS
        if clave.value not in config.NO_MOSTRAR
    }
